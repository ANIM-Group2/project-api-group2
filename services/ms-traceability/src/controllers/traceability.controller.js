...
const getAllLogs = async (req, res) => {
  try {
    ...
    res.json(await svc.getAllLogs({ limit: Number(limit), type, from, to }));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
};

// POST /traceability/event — inter-service event logging
const logEvent = async (req, res) => {
  try {
    res.json(await svc.logEvent(req.body));
  } catch (e) {
...
module.exports = {
  getBatchTrace, getIncidentTrace, getOrderTrace,
  getMaterialTrace, getDashboard, getAllLogs, logEvent,
};